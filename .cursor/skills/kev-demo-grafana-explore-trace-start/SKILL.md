...
After `READY`, use `demo-script-short.md` while driving; fall back to `demo-script.md` for full prompts / narrative.

## Use Case 1 — No data → diagnose & fix the query (Steps 3–6)

### 3. Product context (the 2 a.m. page)

Frame Explore as the ad-hoc **incident investigation** surface (not a saved dashboard), then tell the on-call story from NOTES Beat 1: a paged engineer runs a 5xx-rate query and hits a dead-end **No data** empty state. Full narrative + talk track live in `scripts/demos/explore-trace/demo-script.md`.
